...
screen say(who, what):
    ...
    window:
        ...
        if who is not None:
            ...
            if yumi_perspective == 1:
                window:
                    style "namebox_yumi"
                    text who id "who"
                    $ style.say_window = style.window_yumi

            else:
                window:
                    style "namebox_normal"
                    text who id "who"
...
style window_yumi is window:
    background Image("gui/textbox_yumi.png", xalign=0.5, yalign=1.0)

style window_normal is window:
    background Image("gui/textbox.png", xalign=0.5, yalign=1.0)
...
style namebox_yumi is namebox:
    background Frame("gui/namebox_yumi.png", gui.namebox_borders, tile=gui.namebox_tile, xalign=gui.name_xalign)

style namebox_normal is namebox:
    background Frame("gui/namebox.png", gui.namebox_borders, tile=gui.namebox_tile, xalign=gui.name_xalign)
...
screen navigation():
    ...
    if main_menu:
        hbox:
            ...
            pos(50, 0.9) ## Coordenadas en formato (X, Y)
            spacing 5 ## Espaciado en píxeles. Quitar si no quieres espaciado.

            if main_menu:
                textbutton _("Nuevo juego"):
                    focus_mask None
                    action If(persistent.playername, true=Start(), false=Show(screen="name_input", message= _("Por favor, introduce tu nombre."), defaultname_action=Function(DefaultName), ok_action=Function(FinishEnterName)))
            ...
            if renpy.variant("pc") or (renpy.variant("web") and not renpy.variant("mobile")) and main_menu:
                if persistent.language == "english":
                    textbutton _("Ayuda") action [Help("doc/Game_Manual_En.pdf"), Show(screen="dialog", message=_("El manual de ayuda se abrio en tu navegador."), ok_action=Hide("dialog"))]
                elif persistent.language == None:
                    textbutton _("Ayuda") action [Help("doc/Game_Manual.pdf"), Show(screen="dialog", message=_("El manual de ayuda se abrio en tu navegador."), ok_action=Hide("dialog"))]
        ...
        vbox:
            style_prefix "navigation"
            pos(50, 180) ## Coordenadas en formato (X, Y)
            spacing 5 ## Espaciado en píxeles.
...
screen main_menu():
    ...
    if gui.show_name:
        ...
        vbox:
            text "Ren'Py [renpyversionl]":
                style "main_menu_version"
            #text "[config.version]":
                #style "main_menu_version"
            text "[config.name!t] [config.version] [game_status]":
                style "main_menu_version"

    if config.developer == True:
        if persistent.patch_enabled:
            text _("Extras 1 encontrado."):
                style "extras_00"
        if persistent.patch2_enabled:
            text _("Extras 2 encontrado."):
                style "extras_01"
...
screen game_menu(title, scroll=None, yinitial=0.0):
    ...
    frame:
        if main_menu:
            ...
            style "game_menu_outer_frame_game"
        ...
        hbox:
            ...
            if main_menu:
                ...
                frame:
                    style "game_menu_navigation_frame_in_game"
...
screen about():
    ...
    use game_menu(_("Acerca de"), scroll="viewport"):
        ...
        vbox:
            ...
            if renpy.windows:
                text "{size=-2}{color=#5053a1}Alone [config.version!t].[game_status!t]_renpy.[renpyversionl]_Win{/color}{/size}\n"
            if renpy.linux:
                text "{size=-2}{color=#5053a1}Alone [config.version!t].[game_status!t]_renpy.[renpyversionl]_Linux{/color}{/size}\n"
            if renpy.macintosh:
                text "{size=-2}{color=#5053a1}Alone [config.version!t].[game_status!t]_renpy.[renpyversionl]_Mac{/color}{/size}\n"
            if renpy.android:
                text "{size=-2}{color=#5053a1}Alone [config.version!t].[game_status!t]_renpy.[renpyversionl]_android{/color}{/size}\n"
            ...
            hbox:
                ...
                if main_menu:
                    ...
                    vbox:
                        ...
                        textbutton _("Seleccionar..."):
                            action Show(screen="language", message=_("Estos son los idiomas que están actualmente disponibles."), ok_action=Hide(screen="language")) 
                vbox:
                    style_prefix "check"
                    label _("Misceláneas")
                    if main_menu:
                    ...
                    textbutton _("Apariencia y notificaciones"):
                        action Show(screen="aparience_notify", message= _("{size=+5}Apariencia y notificaciones{/size}"), ok_action=Hide(screen="aparience_notify"))
...
transform -1 notify_appear:
    on show:
        xoffset -100
        alpha 0 
        ease .32 alpha 1.0 xoffset 0
    on hide:
        ease .7 alpha 0.0 xoffset -100
...
screen navigation():
    ...
    if main_menu:
        vbox:
            style_prefix "navigation"
            pos(50, 500) ## Coordenadas en formato (X, Y)
            spacing 20 ## Espaciado en píxeles. Quitar si no quieres espaciado.
